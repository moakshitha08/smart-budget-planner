...
// Update expense - NEW FUNCTION
function updateExpense(id, category, amount, date, description) {
    const user = getCurrentUser();
    if (!user) throw new Error('User not logged in');
    
    const allExpenses = getFromStorage(STORAGE_KEYS.EXPENSES) || [];
    const expenseIndex = allExpenses.findIndex(e => e.id === id && e.userId === user.id && !e.isDeleted);
    ...
    // Update expense
    allExpenses[expenseIndex].category = category;
    allExpenses[expenseIndex].amount = parseFloat(amount);
    allExpenses[expenseIndex].date = date;
    allExpenses[expenseIndex].description = description;
    allExpenses[expenseIndex].budgetType = getBudgetType(category);
    allExpenses[expenseIndex].updatedAt = new Date().toISOString();
    
    // Handle aggregation - if another expense has same category, merge
    const sameCategoryExpenses = allExpenses.filter(e => 
        e.userId === user.id && 
        e.category === category && 
        e.id !== id && 
        !e.isDeleted
    ...
    if (sameCategoryExpenses.length > 0) {
        // Merge into first same category expense
        const mergeTarget = sameCategoryExpenses[0];
        mergeTarget.amount += parseFloat(amount);
        mergeTarget.updatedAt = new Date().toISOString();
        
        // Remove others and current edited one
        allExpenses.splice(expenseIndex, 1);
        const mergeIndex = allExpenses.findIndex(e => e.id === mergeTarget.id);
        if (mergeIndex !== -1) {
            allExpenses[mergeIndex] = mergeTarget;
        }
    }
...
function checkCategoryLimitStatus(category) {
    const limits = getCategoryLimits();
    if (!limits[category]) return { status: 'no-limit', spent: 0, limit: 0, percent: 0 };
    ...
    if (percent > 100) {
        return { status: 'exceeded', spent, limit, percent, nearLimit: false };
    } else if (percent >= 80) {
        return { status: 'near-limit', spent, limit, percent, nearLimit: true };
    } else {
        return { status: 'okay', spent, limit, percent, nearLimit: false };
    }
}

function checkCategoryLimitExceeded(category) {
    const status = checkCategoryLimitStatus(category);
    return status.status === 'exceeded';
}

function getAllCategoryStatuses() {
    const limits = getCategoryLimits();
    const statuses = {};
    
    Object.keys(limits).forEach(function(category) {
        statuses[category] = checkCategoryLimitStatus(category);
    });
